...
export function calculateCardiovascularMetrics(sessions: BloodPressureSession[]): CardiovascularMetrics {
  ...
  const elevatedSystolicSessions = sessions.filter(
    (session) => session.averageSystolic >= HOME_BP_SYSTOLIC_THRESHOLD
  ).length;
  const elevatedDiastolicSessions = sessions.filter(
    (session) => session.averageDiastolic >= HOME_BP_DIASTOLIC_THRESHOLD
  ).length;
  const elevatedSessions = sessions.filter(
    (session) => session.averageSystolic >= HOME_BP_SYSTOLIC_THRESHOLD ||
      session.averageDiastolic >= HOME_BP_DIASTOLIC_THRESHOLD
  ).length;
