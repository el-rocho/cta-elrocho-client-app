...
export interface TreatmentTargetDefinition {
  systolicMin?: number;
  systolicMax: number;
  diastolicMin?: number;
  diastolicMax: number;
  source: 'guideline' | 'custom';
  profile: GuidelineProfile;
}

export interface TreatmentTargetAssessment {
...
export function getGuidelineTreatmentTarget(profile: GuidelineProfile, patientAge: number | null = null): TreatmentTargetDefinition {
  ...
  if (profile === 'ish-2020' && patientAge !== null && patientAge >= 65) {
    return { systolicMin: 120, systolicMax: 139, diastolicMin: 70, diastolicMax: 89, source: 'guideline', profile };
  }
...
export function getTreatmentTarget(settings: AppSettings): TreatmentTargetDefinition {
  if (settings.treatmentTargetMode === 'custom' && hasValidCustomTarget(settings)) {
    return { systolicMin: settings.customTargetSystolicMin || undefined, systolicMax: settings.customTargetSystolicMax, diastolicMin: settings.customTargetDiastolicMin || undefined, diastolicMax: settings.customTargetDiastolicMax, source: 'custom', profile: settings.guidelineProfile };
  }
