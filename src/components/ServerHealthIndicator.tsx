...
export const ServerHealthIndicator: React.FC<ServerHealthIndicatorProps> = ({ healthUrl, isAdmin = false }) => {
  ...
  const indicator = isAdmin ? (
    <button
      type="button"
      className={`server-health-indicator server-health-button status-${status}`}
      aria-label={`${label}. ${t('header.serverHealth.detailsTooltip')}`}
      data-tooltip={`${label}. ${t('header.serverHealth.detailsTooltip')}`}
    ...
    <span
      className={`server-health-indicator status-${status}`}
      role="status"
      aria-label={label}
      tabIndex={0}
      data-tooltip={label}
