...
import {
  ...
  ServerStatusRequestError,
  type ServerStatusErrorCode,
...
const MetricCard: React.FC<MetricCardProps> = ({ icon, label, value, detail, percentage }) => (
  <div className="server-status-metric-card">
    <div className="server-status-metric-heading">
      {icon}
      <span>{label}</span>
    </div>
    ...
    {percentage !== null && percentage !== undefined && (
      <div
        className="server-status-progress"
        role="progressbar"
        aria-label={label}
        aria-valuemin={0}
        aria-valuemax={100}
...
export const ServerStatusModal: React.FC<ServerStatusModalProps> = ({ isOpen, healthUrl, onClose }) => {
  ...
  const memoryValue = status?.memory
    ? status.memory.usagePercent === null
      ? formatBytes(status.memory.usedBytes)
      : `${status.memory.usagePercent.toFixed(1)} %`
    : t('header.serverStatus.notAvailable');
  const memoryDetail = status?.memory
    ? status.memory.limitBytes
      ? t('header.serverStatus.usedOf', {
          used: formatBytes(status.memory.usedBytes),
          total: formatBytes(status.memory.limitBytes),
        })
  ...
  return createPortal(
    <div className="modal-overlay server-status-overlay" onClick={onClose}>
      <div
        className="modal-content server-status-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="modal-header server-status-header">
          <div className="modal-title-group">
            ...
            <div>
              <h2 id="server-status-title">{t('header.serverStatus.title')}</h2>
              <span className="server-status-auto-refresh">{t('header.serverStatus.autoRefresh')}</span>
            </div>
          </div>
          <div className="server-status-header-actions">
            <button
              type="button"
              className="server-status-refresh-button"
              title={t('header.serverStatus.refresh')}
              aria-label={t('header.serverStatus.refresh')}
              onClick={() => void loadStatus()}
              disabled={isLoading}
            >
              <RefreshCw size={18} className={isLoading ? 'is-spinning' : ''} />
            </button>
            <button type="button" className="btn-close-modal" aria-label={t('header.serverStatus.close')} onClick={onClose}>
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        {error && (
          <div className="server-status-error" role="alert">
            {t(`header.serverStatus.${error}`)}
          </div>
        )}
        ...
        {!status && isLoading && (
          <div className="server-status-loading">
            ...
            <span>{t('header.serverStatus.loading')}</span>
          </div>
        )}
        ...
        {status && (
          <div className="server-status-body">
            <div className="server-status-overview">
              <div className="server-status-service-state">
                <span className="server-status-online-dot" aria-hidden="true" />
                <div>
                  <strong>{t('header.serverStatus.serviceAvailable')}</strong>
                  <span>{t('header.serverStatus.databaseAvailable')}</span>
                </div>
              </div>
            ...
            <div className="server-status-metrics-grid">
              <MetricCard
                ...
                label={t('header.serverStatus.cpuProcess')}
                value={status.cpu?.usagePercent === null || !status.cpu
                  ? t('header.serverStatus.calculating')
                  : `${status.cpu.usagePercent.toFixed(1)} %`}
                percentage={status.cpu?.usagePercent}
                detail={status.cpu ? t('header.serverStatus.logicalCores', { count: status.cpu.logicalCores }) : undefined}
              />
              <MetricCard
                icon={<MemoryStick size={18} />}
                label={status.memory?.scope === 'container'
                  ? t('header.serverStatus.memoryContainer')
                  : t('header.serverStatus.memoryProcess')}
                value={memoryValue}
                percentage={status.memory?.usagePercent}
                detail={memoryDetail}
              />
              <MetricCard
                icon={<HardDrive size={18} />}
                label={t('header.serverStatus.diskData')}
                value={status.disk?.usagePercent === null || !status.disk
                  ? t('header.serverStatus.notAvailable')
                  : `${status.disk.usagePercent.toFixed(1)} %`}
                percentage={status.disk?.usagePercent}
                detail={status.disk
                  ? t('header.serverStatus.freeSpace', { free: formatBytes(status.disk.freeBytes) })
                  : undefined}
              />
            </div>
            ...
            <p className="server-status-updated">
              {t('header.serverStatus.updatedAt', { date: formatDate(status.checkedAt) })}
            </p>
          </div>
        )}
      </div>
