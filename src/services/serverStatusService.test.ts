...
describe('getAdminServerStatus', () => {
  it('uses the protected endpoint and sends the active session token', async () => {
    ...
    const result = await getAdminServerStatus('https://server.test/api/health');

    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringMatching(/^https:\/\/server\.test\/api\/admin\/system-status\?fresh=/),
  ...
  it('rejects missing URLs and unauthorized responses', async () => {
    vi.stubGlobal('localStorage', { getItem: vi.fn(() => null) });
    await expect(getAdminServerStatus('')).rejects.toMatchObject({ code: 'unavailable' });
