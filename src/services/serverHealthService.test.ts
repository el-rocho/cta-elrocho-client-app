...
describe('checkServerHealth', () => {
  it('accepts only a successful health response', async () => {
    ...
    await expect(checkServerHealth('https://server.test/api/health')).resolves.toBe(true);
    expect(fetchMock).toHaveBeenCalledWith(
      'https://server.test/api/health',
  ...
  it.each([
    ...
    await expect(checkServerHealth('https://server.test/api/health')).resolves.toBe(false);
  });
  ...
  it('uses the existing auth endpoint with a 1.6.0 server', async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      'https://server.test/api/auth/status',
      expect.objectContaining({ cache: 'no-store' })
    );
  });
});
